...
template<typename T>
constexpr bool is_gpu_compatible = std::is_standard_layout_v<T> && alignof(T) >= 4
...
public:
    ...
    /// Construct from byte offset with matching const qualification.
    constexpr explicit TypedOffset(ByteOffset byte_offset) noexcept: TypedOffset(byte_offset.get())
    {}

    /// Convert to byte offset with matching const qualification.
    constexpr explicit operator ByteOffset() const noexcept
    { return ByteOffset(m_value); }
